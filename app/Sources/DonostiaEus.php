...
class DonostiaEus {
	...
	private function _addCategoriesFromUrlAndTitle($urlCat, $title) {
		...
		if (Functions::strposArray($haystack, $theatreSlugs)!==false) $categories[] = 'Teatro y Danza';

		// Cinema
		$cinemaSlugs = array('cine', 'película', 'film');
		if (Functions::strposArray($haystack, $cinemaSlugs)!==false) $categories[] = 'Cine';

		// Music / Concerts
		$musicSlugs = array('concierto', 'kontzertu','música', 'musika');
		if (Functions::strposArray($haystack, $musicSlugs)!==false) $categories[] = 'Música';

		// Sports
		$sportSlugs = array('deporte', 'kirol', 'carrera', 'karrera', 'maratón', 'maratoia');
		if (Functions::strposArray($haystack, $sportSlugs)!==false) $categories[] = 'Deportes';

		// Expositions
		$expositionSlugs = array('exposicion', 'exposición', 'erakusketa');
		if (Functions::strposArray($haystack, $expositionSlugs)!==false) $categories[] = 'Exposiciones';

		// Parties
		$partySlugs = array('fiesta', 'feria', 'festa', 'azoka', 'mercadillo');
		if (Functions::strposArray($haystack, $partySlugs)!==false) $categories[] = 'Fiestas y Ferias';

		// Gastronomy
		$gastronomySlugs = array('gastro', 'pinchos', 'pintxoak');
		if (Functions::strposArray($haystack, $gastronomySlugs)!==false) $categories[] = 'Gastronomía';

		// Children
		$childrenSlugs = array('infantil', 'haur', 'famili');
		if (Functions::strposArray($haystack, $childrenSlugs)!==false) $categories[] = 'Actividades Infantiles';

		// Museums
		$museumSlugs = array('museo');
		if (Functions::strposArray($haystack, $museumSlugs)!==false) $categories[] = 'Museos';

		// Conferences
		$conferenceSlugs = array('conferencia', 'hitzaldi', 'konferentzia');
		if (Functions::strposArray($haystack, $conferenceSlugs)!==false) $categories[] = 'Conferencias';

		// Literature
		$literatureSlugs = array('literatura', 'lector', 'irakur');
		if (Functions::strposArray($haystack, $literatureSlugs)!==false) $categories[] = 'Literatura';
